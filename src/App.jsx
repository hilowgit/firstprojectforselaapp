...
const Header = ({ onMenuToggle }) => (
  <header className="fixed top-0 right-0 left-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm shadow-md h-16 z-40 flex items-center justify-between px-4">
    <h1 className="text-xl font-bold text-blue-700 dark:text-sky-400">المركز التعليمي</h1>
    <button onClick={onMenuToggle} className="p-2 text-slate-800 dark:text-slate-200" aria-label="فتح القائمة">
...
const SideMenu = ({ isOpen, onClose, theme, setTheme }) => {
  ...
  return (
    <>
      <div onClick={onClose} className="fixed inset-0 bg-black/60 z-40 animate-fade-in" aria-hidden="true"></div>
      <aside 
        className="fixed top-0 right-0 bottom-0 w-72 bg-gray-50 dark:bg-slate-800 shadow-2xl z-50 transition-transform duration-300 ease-in-out"
        style={{ transform: isOpen ? 'translateX(0)' : 'translateX(100%)' }}
      >
        <div className="p-4">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">الإعدادات</h2>
            <button onClick={onClose} className="p-2 text-slate-600 dark:text-slate-400" aria-label="إغلاق القائمة">
               <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
          </div>
          <ul>
            <li>
              <button onClick={toggleTheme} className="w-full flex items-center gap-4 p-3 rounded-lg text-lg text-slate-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700">
                {theme === 'dark' ? 
...
const CourseDetails = ({ course, onBack }) => { const [currentImageIndex, setCurrentImageIndex] = useState(0); const nextImage = () => { setCurrentImageIndex((prev) => (prev + 1) % course.images.length); }; const prevImage = () => { setCurrentImageIndex((prev) => (prev - 1 + course.images.length) % course.images.length); }; const CountdownTimer = ({ endDate }) => { const calculateTimeLeft = () => { const diff = +new Date(endDate) - +new Date(); let timeLeft = {}; if (diff > 0) { timeLeft = { 'أيام': Math.floor(diff / (1000 * 60 * 60 * 24)), 'ساعات': Math.floor((diff / (1000 * 60 * 60)) % 24), 'دقائق': Math.floor((diff / 1000 / 60) % 60) }; } return timeLeft; }; const [timeLeft, setTimeLeft] = useState(calculateTimeLeft()); useEffect(() => { const timer = setTimeout(() => { setTimeLeft(calculateTimeLeft()); }, 1000); return () => clearTimeout(timer); }, [timeLeft]); const components = Object.keys(timeLeft).map((i) => <div key={i} className="text-center"><div className="text-xl md:text-2xl font-bold text-blue-600 dark:text-sky-400">{timeLeft[i]}</div><div className="text-xs text-slate-500 dark:text-slate-400">{i}</div></div>); return components.length ? <div className="flex justify-center gap-4">{components}</div> : <span className="text-red-500 font-semibold">انتهى العرض!</span>; }; const handleWhatsAppInquiry = () => { const phone = '966000000000'; const msg = `مرحباً، أرغب بالاستفسار عن دورة "${course.name}". هل يمكنني الحصول على المزيد من التفاصيل؟`; window.open(`https://wa.me/${phone}?text=${encodeURIComponent(msg)}`, '_blank'); }; return ( <div className="w-full max-w-4xl mx-auto p-4 md:p-6 bg-white dark:bg-slate-800 rounded-lg animate-fade-in shadow-xl"><button onClick={onBack} className="mb-4 flex items-center gap-2 text-blue-600 dark:text-sky-500 hover:text-blue-700 dark:hover:text-sky-400 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>العودة لقائمة الدورات</button><h1 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-4 text-center">{course.name}</h1><div className="relative w-full max-w-2xl mx-auto mb-6 rounded-lg overflow-hidden shadow-lg"><img src={course.images[currentImageIndex]} alt={`صورة عرض ${currentImageIndex + 1}`} className="w-full h-56 md:h-72 object-cover transition-transform duration-500 ease-in-out" />{course.images.length > 1 && (<><button onClick={prevImage} className="absolute top-1/2 left-2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full hover:bg-opacity-75 transition-opacity"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6"/></svg></button><button onClick={nextImage} className="absolute top-1/2 right-2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full hover:bg-opacity-75 transition-opacity"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg></button></>)}</div><div className="space-y-6"><div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-lg"><h2 className="text-2xl font-semibold text-blue-700 dark:text-sky-400 mb-3">ماذا ستتعلم؟</h2><ul className="space-y-2 pr-5 text-slate-700 dark:text-slate-300 list-disc list-inside">{course.outline.map((item, i) => <li key={i}>{item}</li>)}</ul></div><div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-lg"><h2 className="text-2xl font-semibold text-blue-700 dark:text-sky-400 mb-3">لمن هذه الدورة؟</h2><ul className="space-y-2 pr-5 text-slate-700 dark:text-slate-300 list-disc list-inside">{course.prerequisites.map((item, i) => <li key={i}>{item}</li>)}</ul></div><div className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-lg text-center flex justify-around items-center"><div><p className="text-lg font-semibold text-slate-800 dark:text-white">المدة الإجمالية</p><p className="text-xl text-blue-600 dark:text-sky-400">{course.duration}</p></div><div className="border-l-2 border-gray-200 dark:border-slate-600 h-12"></div><div><p className="text-lg font-semibold text-slate-800 dark:text-white">عدد الجلسات</p><p className="text-xl text-blue-600 dark:text-sky-400">{course.sessions}</p></div></div><div className="bg-blue-100 dark:bg-slate-700 p-5 rounded-lg text-center shadow-lg"><h2 className="text-2xl font-bold text-blue-800 dark:text-white mb-2">عرض خاص!</h2><div className="flex justify-center items-center gap-4 my-3"><span className="line-through text-slate-500 dark:text-slate-400 text-xl">{course.price.original} ريال</span><span className="text-3xl font-extrabold text-blue-600 dark:text-yellow-300 animate-pulse">{course.price.discounted} ريال فقط!</span></div><div className="mt-4 bg-white/50 dark:bg-slate-800/50 p-3 rounded-lg"><p className="mb-2 text-slate-700 dark:text-slate-200">ينتهي العرض خلال:</p><CountdownTimer endDate={course.offerEndDate} /></div></div><div className="mt-8"><button onClick={handleWhatsAppInquiry} className="w-full flex items-center justify-center gap-3 bg-green-500 hover:bg-green-600 text-white font-bold py-4 px-6 rounded-lg text-xl transition-transform transform hover:scale-105 shadow-lg"><svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="currentColor"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.894 11.892-1.99 0-3.903-.52-5.586-1.456l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.451-4.437-9.887-9.888-9.888-5.452 0-9.888 4.436-9.889 9.888.001 2.228.651 4.315 1.731 6.086l.001.004l-1.043 3.803 3.827-1.011z"/></svg>سجل الآن عبر الواتساب</button></div></div></div> ); };
const CoursesList = ({ category, onSelectCourse, onBack }) => { return ( <div className="w-full max-w-4xl mx-auto p-4 md:p-6 bg-white dark:bg-slate-800 rounded-lg animate-fade-in shadow-xl"><button onClick={onBack} className="mb-4 flex items-center gap-2 text-blue-600 dark:text-sky-500 hover:text-blue-700 dark:hover:text-sky-400 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>العودة للأقسام</button><h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-6 text-center">دورات {category.name}</h1>{category.courses.length > 0 ? ( <div className="space-y-4">{category.courses.map((course) => ( <div key={course.id} onClick={() => onSelectCourse(course.id)} className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-lg flex justify-between items-center cursor-pointer hover:bg-blue-100 dark:hover:bg-slate-700 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1"><div><h2 className="text-xl font-semibold text-blue-700 dark:text-sky-400">{course.name}</h2><p className="text-slate-600 dark:text-slate-400">{course.shortDescription}</p></div><svg className="text-slate-400 dark:text-slate-500" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg></div> ))}</div> ) : ( <p className="text-center text-slate-500 dark:text-slate-400 mt-8 text-lg">لا توجد دورات في هذا القسم حالياً. ترقبوا جديدنا!</p> )}</div> ); };
const HomeScreen = ({ onSelectCategory }) => { return ( <div className="w-full max-w-5xl mx-auto p-4 md:p-6 bg-white dark:bg-slate-800 rounded-lg animate-fade-in shadow-xl"><h1 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-2 text-center">أقسامنا التدريبية</h1><p className="text-center text-slate-600 dark:text-slate-400 mb-8">اختر اهتمامك وانطلق في رحلة التعلم</p><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{educationalCenterData.categories.map((category) => { const { IconComponent } = category; return ( <div key={category.id} onClick={() => onSelectCategory(category.id)} className="bg-slate-50 dark:bg-slate-700/50 p-6 rounded-lg text-center cursor-pointer flex flex-col items-center justify-center gap-4 hover:bg-blue-100 dark:hover:bg-slate-700 hover:shadow-xl hover:border-blue-500 dark:hover:border-sky-500 border-2 border-transparent transition-all duration-300 transform hover:-translate-y-2"><div className="text-blue-600 dark:text-sky-400"><IconComponent className="w-12 h-12" /></div><h2 className="text-xl font-semibold text-slate-800 dark:text-white">{category.name}</h2></div> ); })}</div></div> ); };
...
export default function App() {
  ...
  return (
    <div className="bg-slate-100 dark:bg-slate-900 min-h-screen text-slate-800 dark:text-slate-200 font-['Cairo',_sans-serif] transition-colors duration-300">
      ...
      {installPrompt && (
        <button onClick={handleInstallClick} className="fixed bottom-4 right-4 bg-blue-600 dark:bg-sky-500 hover:bg-blue-700 dark:hover:bg-sky-600 text-white font-bold py-3 px-5 rounded-full shadow-lg z-30 flex items-center gap-3 animate-pulse transition-transform transform hover:scale-110" title="تثبيت التطبيق على جهازك">
